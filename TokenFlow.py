...
def produceToken(net_P, node, time, fourToken, pd = '0', w = '1'):
...
def protoListFlow(petriNet_P, petriNet_A1, petriNet_A2, petriNet_L, fourToken, inputSeq):
    for p in petriNet_P:
        if p.split('*')[1] == 'Start':
    ...
    for [protocol, time] in inputSeq:
        for arcFrom in petriNet_A1:
            if arcFrom[1] == protocol:
                consumeToken(petriNet_P, arcFrom[0], time, fourToken, arcFrom[2], arcFrom[0].split('*')[0])
        for arcTo in petriNet_A2:
            if arcTo[0] == protocol:
                produceToken(petriNet_P, arcTo[1] , time, fourToken, arcTo[2], arcFrom[1].split('*')[0])
...
def tokenReplay():
    for root, dirs, files in os.walk('/home/jin/Documents/Generated Data/Grouped Chain'):
                    ...
                    for (j, l) in enumerate(reader):
                        ...
                        if (j == 0):
                            ...
                            if l['Protocol'] in petriNet_T:
                                ...
                                fitness = protoListFlow(petriNet_P, petriNet_A1, petriNet_A2, petriNet_L, fourToken, inputSeq)
                                print(fourToken)
                                if fitness > 0.8:
                                    chainNum = file.split('_')[3]
                                    attackSeq.append([chainNum, fitness, i, stepNum, inputSeq])
                                    print([chainNum, fitness, i, stepNum, inputSeq])
                                    inputSeq = []
                                    if stepNum < len(petriNet_T)-1:
                                        stepNum = stepNum + 1
                                    else:
        ...
        if len(attackSeq) > 0:
            ...
            data.to_csv('/home/jin/Documents/Generated Data/Attack Sequence/sequence data')
