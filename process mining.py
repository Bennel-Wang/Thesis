...
def calfitness(sixToken,vdict):
    ...
    n = sixToken[4]
    a = sixToken[5]
    fitness = 1/2 * (1 - ((m+x)/(c+x))) + 1/2 * (1 - ((r+x)/(p+x))) #+ 1/3 * (1 - 10*((n+x)/(a+x)))    #for numerator and denominator all 0, fitness = 0
    s = 0
...
def processflow(groupNum, Protocol, sixTlist, TVdictlist, Pdictlist):
    ...
    if Protocol not in TLdict:
        ...
        TVdictlist[groupNum][Protocol] = True   #mark as visited
        ...
        sixTlist[groupNum][3] = sixTlist[groupNum][0] + sixTlist[groupNum][2] - sixTlist[groupNum][1]
        return calfitness(sixTlist[groupNum],TVdictlist[groupNum])
                ...
                if (SrcIpFreq[l['SrcIp']] == 0):                    #SrcIp not as SrcIp before
                    ...
                    if (DesIpFreq[l['SrcIp']] == 0):                #and SrcIp not as DesIp before
                        ...
                        if (validTimeGap(IpLastTime[l['SrcIp']], l['Time'], validInterval)):    #if the time interval between Ip current and last appearance, link the path
                            ...
                            fitness = processflow(IpGroup[l['SrcIp']], l['Protocol'], sixTlist, visitTgroup, tokenNumgroup)
